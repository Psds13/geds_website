...
export default function PortfoliosPage() {
  return (
    <section className="bg-background text-foreground py-20 min-h-screen relative">
      ...
      <SquareReveal gridSize={12}>
        <div className="container mx-auto px-4 max-w-7xl relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {portfolioProjects.map((project, index) => (
              <motion.div
                ...
                className="group relative bg-foreground/5 rounded-3xl overflow-hidden shadow-2xl border border-foreground/10 hover:border-cyan-500/50 transition-all duration-500 hover:shadow-[0_0_40px_rgba(0,219,255,0.15)] backdrop-blur-sm"
              >
                <Link href={project.link} target="_blank" rel="noopener noreferrer" className="block relative h-72 md:h-80 w-full overflow-hidden">
                  ...
                  />
                  <div className="absolute inset-0 bg-linear-to-t from-background via-background/40 to-transparent opacity-80 group-hover:opacity-60 transition-opacity duration-500" />

                  <div className="absolute top-4 right-4 bg-background/50 backdrop-blur-md p-3 rounded-full border border-foreground/10 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                    <FiExternalLink className="text-cyan-400 text-xl" />
                  </div>
                </Link>

                <div className="p-8 md:p-10 relative">
                  <div className="absolute top-0 left-10 w-20 h-1 bg-linear-to-r from-cyan-500 to-blue-500 transform -translate-y-1/2"></div>

                  <div className="flex justify-between items-start mb-4">
                    <h2 className="text-3xl font-bold text-foreground group-hover:text-cyan-500 dark:group-hover:text-cyan-400 transition-colors">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-foreground/70 uppercase tracking-widest">Tecnologias</h3>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {project.techs.map((tech, i) => (
                        <span
                          key={i}
                          className="bg-background/40 text-cyan-600 dark:text-cyan-300 text-sm font-medium px-4 py-1.5 rounded-full border border-foreground/10 group-hover:border-cyan-500/30 transition-colors"
                        >
                          {tech}
                        </span>
                      ))}
                  ...
                  <div className="mt-10">
                    <Link
                      ...
                      className="inline-flex items-center text-foreground hover:text-black hover:bg-cyan-500 dark:hover:bg-cyan-400 border border-foreground/20 hover:border-transparent px-8 py-3 rounded-full font-semibold transition-all duration-300"
                    >
                      Acessar o Projeto
          ...
          <motion.div
            ...
            className="mt-24 text-center border-t border-foreground/10 pt-16"
          >
            <h3 className="text-2xl font-bold mb-4 text-foreground">Quer ver sua ideia transformar-se em realidade?</h3>
            <p className="text-foreground/70 text-lg font-light mb-8 max-w-2xl mx-auto">
