      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[280px]">
        {diferenciais.map((item, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            whileHover={{ y: -5 }}
            className={`group relative bg-background border border-foreground/5 hover:border-cyan-500/30 rounded-[2.5rem] p-10 transition-all duration-500 flex flex-col justify-end overflow-hidden shadow-2xl ${
          ...
          >
            ...
            <div className="absolute top-10 left-10 w-16 h-16 bg-foreground/5 border border-foreground/10 rounded-2xl flex items-center justify-center text-cyan-600 dark:text-cyan-400 group-hover:bg-cyan-500 group-hover:text-white dark:group-hover:text-black group-hover:scale-110 transition-all duration-500 z-20 shadow-lg">
              {item.icon}
            </div>

            <div className="relative z-30">
              <h3 className="text-foreground font-black text-xl mb-3 tracking-tighter group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors uppercase italic leading-none">
                {item.title}
              </h3>
              <p className="text-foreground/60 text-[11px] leading-relaxed font-bold tracking-tight group-hover:text-foreground/80 transition-colors uppercase opacity-80">
                {item.desc}
              </p>
            </div>
          </motion.div>
