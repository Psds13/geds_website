...
export default function HackerMode() {
  ...
  const [output, setOutput] = useState<{ id: string; text: string; color?: string }[]>([
    { id: "1", text: "GEDS OS [Version 1.0.4.15]", color: "text-emerald-500" },
    { id: "2", text: "(c) GEDS Inovação. Todos os direitos reservados.", color: "text-emerald-500" },
    { id: "3", text: "Type 'help' for available commands or 'exit' to close.", color: "text-emerald-300" }
  ]);
  
  // Konami Code / "geds" listener
  useEffect(() => {
    let keyBuffer = "";
    const secretWord = "geds";
  ...
  const handleCommand = (cmd: string) => {
    ...
    const newOut = [...output, { id: Math.random().toString(), text: `C:\\GEDS> ${cmd}` }];
    ...
    if (c === "help") {
      ...
      newOut.push({ id: Math.random().toString(), text: "ACCESS DENIED. Intrusion attempt logged. Sending IP to GEDS Security...", color: "text-red-500" });
    } else if (c === "clear") {
      setOutput([]);
      setInput("");
      return;
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 bg-emerald-950/40 border-b border-emerald-900/50">
            <div className="flex items-center gap-2">
              ...
              <span className="text-emerald-500 font-bold tracking-widest text-sm">GEDS_TERMINAL_ROOT</span>
            </div>
          ...
          <div className="flex-1 p-6 overflow-y-auto text-emerald-500 text-sm md:text-base space-y-2">
            ...
            <div className="flex items-center mt-2">
              <span className="mr-2">C:\GEDS&gt;</span>
              <input 
                type="text" 
                value={input}
